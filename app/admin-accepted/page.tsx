...
const AdminAccepted = () => {
  ...
  const fetchAccepted = async () => {
    ...
    if (error) {
      console.error(error);
      toast.error('Failed to fetch accepted applicants');
    } else {
  ...
  return (
    <AdminLayout title="Accepted Applicants">
      {loading ? (
        <p className="text-gray-500">Loading accepted candidates...</p>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-green-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-800">Accepted At</th>
                <th className="px-6 py-3 text-right text-sm font-semibold text-gray-800">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {acceptedList.map((candidate) => (
                <tr key={candidate.id}>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500">
                    {new Date(candidate.accepted_at).toLocaleDateString()}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl relative">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Accepted for <span className="font-semibold">{selected.job_title}</span> —{' '}
